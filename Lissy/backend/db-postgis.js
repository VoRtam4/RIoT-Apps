...
async function getRouteDetail(id) {
    try {
        const result = await db_postgis.query(`SELECT id, route_type, route_id, route_short_name,
            route_color, route_text_color FROM routes WHERE id = $1 AND is_active=true LIMIT 1;`, [id]);
...
async function getRoutesDetailByRouteIds(routeIds) {
    ...
    try {
        routes = (await db_postgis.query(`SELECT id, route_type, route_id, route_short_name,
            route_color, route_text_color FROM routes WHERE route_id = ANY($1) AND is_active=true;`, [routeIds])).rows;
    } catch (error) {
        log('error', error);
        return [];
    }
...
async function getTripsDetail(tripIds, fullStopsOrder) {
    ...
    return await groupTripsByShape(result.rows, fullStopsOrder, false);
}

async function getTripsDetailByTripIds(gtfsTripIds, fullStopsOrder) {
    let result;

    if (!Array.isArray(gtfsTripIds) || gtfsTripIds.length < 1) {
        return [];
    }

    try {
        result = await db_postgis.query(`SELECT id, trip_id, shape_id, stops, stops_info
            FROM trips WHERE trip_id = ANY($1) AND is_active=true`, [gtfsTripIds]);
    } catch (error) {
        log('error', error);
        return [];
    }

    return await groupTripsByShape(result.rows, fullStopsOrder, true);
}

async function getTripsDetailByRealtimeRefs(routeId, realtimeRefs, fullStopsOrder) {
    if (typeof routeId !== 'string' || routeId.length < 1 || !Array.isArray(realtimeRefs) || realtimeRefs.length < 1) {
        return [];
    }
    ...
    try {
        result = await db_postgis.query(`SELECT id, trip_id, shape_id, stops, stops_info
            FROM trips WHERE route_id = $1 AND is_active=true`, [routeId]);
    } catch (error) {
        log('error', error);
        return [];
    }
    ...
    const departureTimes = new Set(
        realtimeRefs
            .map((item) => item?.departure_time)
            .filter((item) => typeof item === 'string' && item.length > 0)
    );
    const matchedTrips = result.rows.filter((trip) => {
        if (!Array.isArray(trip.stops) || trip.stops.length < 2 || !Array.isArray(trip.stops_info) || trip.stops_info.length < 1) {
            return false;
        }

        const departureTime = combineGTFSTimes(trip.stops_info[0].aT, trip.stops_info[0].dT);
        return departureTimes.has(departureTime);
    });

    return await groupTripsByShape(matchedTrips, fullStopsOrder, true);
}

async function groupTripsByShape(trips, fullStopsOrder, stripTripId) {
    let result;
    let stops = {};
...
async function getTripDetail(id) {
    try {
        const result = await db_postgis.query(`SELECT id, trip_id, route_id, route_id_id, shape_id, stops, stops_info
            FROM trips WHERE id = $1 AND is_active=true LIMIT 1`, [id]);
        return result.rows[0] ?? null;
    } catch (error) {
        log('error', error);
        return null;
...
async function getFullShape(id) {
    ...
    output['coords'] = sanitizeShapeSegments(output['coords'], output['stops']);

    return output;
}

function sanitizeShapeSegments(coords, stops) {
    const stopCoords = Array.isArray(stops) ? stops.map((stop) => stop.coords).filter((coord) => Array.isArray(coord) && coord.length >= 2) : [];
    const fallbackSegments = [];
    for (let idx = 0; idx < stopCoords.length - 1; idx++) {
        fallbackSegments.push([stopCoords[idx], stopCoords[idx + 1]]);
    }
    ...
    return coords.map((segment, idx) => {
        const validSegment = Array.isArray(segment) && segment.length >= 2;
        if (validSegment) {
            return segment;
        }

        if (fallbackSegments[idx]) {
            return fallbackSegments[idx];
        }

        const start = stopCoords[idx];
        const end = stopCoords[idx + 1];
        if (start && end) {
            return [start, end];
        }
...
module.exports = { connectToDB, reloadNetFiles, addAgency, getActiveAgencies, addStop, getStopPositions,
    ...
    getFullShape, getTripsWithUniqueShape, getRoutesDetail, getRouteDetail, getRoutesDetailByRouteIds,
    getTripsDetail, getTripsDetailByTripIds, getTripsDetailByRealtimeRefs, getTripDetail }
