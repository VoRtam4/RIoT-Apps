...
When the backend is configured for RIoT integration, these delay values are no longer read from the original Lissy operational dataset. The API keeps the same contract, but the values are reconstructed from RIoT time-series KPI evaluations and mapped back to the static trip shapes stored by Lissy.
