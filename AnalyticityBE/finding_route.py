...
def prepare_data_from_path(streets_gdf: GeoDataFrame, route: LineString,  original_streets: list,
    ...
    df_routes = streets_gdf[streets_gdf['is_route']].copy()
    ...
    df_routes_intersection = df_routes[df_routes['not_point_intersection']].copy()
    streets = list(set(df_routes_intersection['nazev'].values.tolist() + original_streets))
    gdf_final = df_routes_intersection.drop(['is_route', 'intersection', 'not_point_intersection'], axis=1)
    return gdf_final, streets
...
def find_route_by_streets(src_street: str, dst_street: str, from_time: str, to_time: str, gdf: GeoDataFrame,
    ...
    gdf_final, streets = prepare_data_from_path(streets_gdf, route, original_streets, from_time, to_time)

    street_geometry_dict = []
    for _, row in gdf_final.iterrows():
        street_geometry_dict.append({
...
def find_route_by_coord(src_coord, dst_coord,
    ...
    street_geometry_dict = []
    route_streets, _ = prepare_data_from_path(streets_gdf, route, [source_street, dst_street], from_time, to_time)
    for _, row in route_streets.iterrows():
        street_geometry_dict.append({
            'street_name': row['nazev'],
            'path': [[long, lat] for lat, long in row['geometry'].coords],
            'color': 'green',
